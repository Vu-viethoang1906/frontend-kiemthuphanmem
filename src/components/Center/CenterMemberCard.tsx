...
interface CenterMemberCardProps {
  member: any;
  isUpdating?: boolean;
  baseUrl: string;
  onClick: () => void;
...
const CenterMemberCard: React.FC<CenterMemberCardProps> = ({
  member,
  isUpdating = false,
  baseUrl,
  onClick,
  ...
  const roleClass = getRoleClass(member.role_in_center);
    ...
    >
      ...
      <div className="mb-3">
        <span className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${roleClass}`}>
          {translateRoleText(member.role_in_center)}
        </span>
      </div>
...
export default CenterMemberCard;
