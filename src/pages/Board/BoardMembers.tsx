      ...
      <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {boards.map((board) => (
          <div
            ...
            className="border border-gray-200 rounded-xl p-5 bg-white shadow-sm hover:shadow-lg transition-all duration-200"
          >
            {/* Title */}
            <h3 className="text-lg font-semibold text-gray-900 mb-2 line-clamp-1">
              {board.board_id?.title || 'Untitled'}
            </h3>

            {/* Info */}
            <div className="space-y-1 text-sm">
              <p className="text-gray-700 break-all">
                <span className="font-medium text-gray-900">ID:</span> {board.board_id?._id}
              </p>

              <p className="text-gray-700">
                <span className="font-medium text-gray-900">Role:</span>{' '}
                <span className="text-blue-600">{board.role_in_board}</span>
              </p>
            ...
            {/* Action */}
            <button
              onClick={() => navigate(`/project/${board.board_id?._id}`)}
              className="mt-4 w-full flex items-center justify-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-md shadow-sm transition-colors"
            >
              Open Board
            </button>
          </div>
